...
def main() -> None:
    ...
    cache = root / "outputs/source_validation/m0_template_cache.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    parser.save_cache(cache)
    result = {
        "smoke_status": "passed", "rows": len(rows), "templates": parser.catalog()["templates"],
        "cache_path": str(cache), "parser_version": parser.VERSION, "ait_accessed": False,
    }
