...
class JsonDirectoryRepository(SecurityRepository):
    ...
    async def get_entity_history(self, entity_id: str, time_range: str) -> RepositoryResult:
        result = await self.search_logs(entities=[entity_id], source_types=[], keywords=[], start_time=None, end_time=None, limit=200)
        if not result.ok:
            return result
        return RepositoryResult(ok=True, data={"entity_id": entity_id, "range": time_range, **result.data}, evidence_refs=result.evidence_refs)
    ...
    async def get_attack_timeline(self, *, finding_ids: list[str], entity_ids: list[str], time_range: str) -> RepositoryResult:
        ...
        matched.sort(key=lambda row: str(row.get("time", row.get("timestamp", ""))))
        matched = matched[:200]
        refs = [str(row.get("event_id", row.get("id", ""))) for row in matched]
